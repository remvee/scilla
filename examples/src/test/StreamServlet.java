...
import org.scilla.ScillaConversionFailedException;
import org.scilla.ScillaException;
import org.scilla.ScillaIllegalRequestException;
import org.scilla.ScillaInputIOException;
import org.scilla.ScillaNoConverterException;
import org.scilla.ScillaNoInputException;
import org.scilla.ScillaNoOutputException;
import org.scilla.ScillaOutputIOException;
...
public class StreamServlet extends HttpServlet {
    ...
    public void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            log.info("doGet! ", ex);
            response.sendError(HttpServletResponse.SC_NOT_IMPLEMENTED, ex
                    .getMessage());
    ...
    /**
     * Add audio info to stream headers using "<TT>icy-name</TT>" and "<TT>
     * x-audiocast-name</TT>".
     * 
     * @param req
     *            scilla request object
     * @param response
    ...
    private void addStreamHeaders(Request req, HttpServletResponse response) {
        try {
            ...
            if (info.getPerformer() != null
                    && info.getPerformer().length() != 0) {
    ...
    private Request createFromHttpServletRequest(HttpServletRequest req)
            throws Exception {
        // source file
        String source = AppConfig.getSourceDir() + File.separator
                + req.getPathInfo();
        if (source.indexOf("/../") != -1) { throw new ScillaIllegalRequestException(); }
        ...
        if (!(new File(source)).exists()) {
            ...
            if (!(new File(source)).exists()) { throw new ScillaException(
                    "can't find source"); }
        }
