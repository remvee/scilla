...
/**
 * This image tag creates an <tt>img</tt> HTML tag to an optionally
 * transformed image with the proper <tt>width</tt> and <tt>height</tt>
 * attributes set.
 * 
 * @version $Id: ImageTag.java,v 1.20 2004/05/02 20:09:34 remco Exp $
...
public class ImageTag extends BodyTagSupport {
    ...
    public int doEndTag() throws JspException {
        HttpServletRequest pageRequest = (HttpServletRequest) pageContext
                .getRequest();
        ...
        if (getOutputtype() == null) {
            try {
                ImageInfo inputInfo = (ImageInfo) InfoFactory.get(getRequest()
                        .getInputFile());
                ...
                if (!inputInfo.isJPEG() && !inputInfo.isGIF()) {
                    ...
                    // handle PNGs
                    boolean acceptPNG = acceptHeader != null
                            && acceptHeader.indexOf("image/png") != -1;
                    if (!acceptPNG && inputInfo.isPNG()) {
                        // indexed or transparent images become gifs others
                        // jpegs
                        preferredOutputtype = inputInfo.isIndexed()
                                || inputInfo.hasAlphaChannel() ? "gif" : "jpg";
                    }
                }
            } catch (Exception ex) {
        ...
        if (var != null) {
            ...
            try {
                ...
                log.error("failed to determine image url", ex);
        ...
        try {
            ...
            try {
                ImageInfo outputInfo = (ImageInfo) InfoFactory.get(getRequest()
                        .getOutputFile());
                if (outputInfo != null) {
    ...
    public String getSrc() throws Exception {
        if (name != null) {
            ...
            absSrc = img.getString(DirectoryBean.LOCATION_KEY);
        }

        if (absSrc != null) { return absSrc; }
        ...
        if (i == -1) { throw new JspException(
                "unable to determine relative location"); }
        ...
        while (t.startsWith("../")) {
            ...
            if (j == -1) { throw new JspException("too many ../"); }
            pageLocation = pageLocation.substring(0, j);
        }

        // security check
        if (t.indexOf("..") != -1) { throw new JspException("illegal location"); }
    ...
    // private stuff
    /**
     * Create a request object from the current source and transformations
     * properties.
     */
    ...
    /**
     * Create an URL to the scilla servlet from the current source and
     * transformations properties.
     */
    private String getImageUrl() throws Exception {
        StringBuffer out = new StringBuffer();

        HttpServletRequest pageRequest = (HttpServletRequest) pageContext
                .getRequest();
        HttpServletResponse pageResponse = (HttpServletResponse) pageContext
                .getResponse();

        out.append(pageRequest.getContextPath());
        out.append(SERVLET_MAPPING);

        out.append(name != null ? ImageServlet.SCILLA_SOURCE_CTX
                : ImageServlet.APPLICATION_CTX);
    ...
    private List getRequestParameters() {
        ...
        if (outputtype != null) {
            result.add(new RequestParameter(Request.OUTPUT_TYPE_PARAMETER,
                    outputtype));
